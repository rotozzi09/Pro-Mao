...
import { Search, ArrowRight, Star, ShieldCheck, MapPin, LogOut, ChevronLeft, HeartHandshake, Plus, X, Share2, Copy, Mail, ExternalLink, MessageCircle, Calendar, UserCog } from "lucide-react";
import "@/App.css";
...
function AppRouter() {
  ...
  return <div className="app-shell">
    <nav className="topbar">
      ...
      <div className="nav-links"><button data-testid="nav-services-button" onClick={() => search(category)}>Encontrar serviço</button>{user && <button data-testid="nav-favorites-button" onClick={() => { setMessage(""); navigate("/"); setView("favorites"); }}>Favoritos</button>}{user && <button data-testid="nav-appointments-button" onClick={() => { setMessage(""); navigate("/"); setView("appointments"); }}>Agendamentos</button>}<button data-testid="nav-how-button" onClick={() => { setMessage(""); navigate("/"); setView("about"); }}>Como funciona</button></div>
