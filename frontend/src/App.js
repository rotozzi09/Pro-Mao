...
import { Search, ArrowRight, Star, ShieldCheck, MapPin, LogOut, ChevronLeft, HeartHandshake, Plus, X, Share2, Copy, Mail, ExternalLink } from "lucide-react";
import "@/App.css";
...
function AppRouter() {
  ...
  return <div className="app-shell">
    <nav className="topbar">
      ...
      <div className="nav-links"><button data-testid="nav-services-button" onClick={() => search(category)}>Encontrar serviço</button><button data-testid="nav-how-button" onClick={() => { setMessage(""); navigate("/"); setView("about"); }}>Como funciona</button></div>
      <div className="nav-actions">{user ? <><button className="profile-chip" data-testid="profile-menu-button" onClick={() => { setMessage(""); navigate("/"); setView("profile"); }}><span className="avatar-small">{user.name?.[0]}</span>{user.name?.split(" ")[0]}</button><button className="icon-btn" data-testid="logout-button" onClick={logout} title="Sair"><LogOut size={18}/></button></> : <><button className="login-link" data-testid="login-open-button" onClick={() => { setMessage(""); setAuth("login"); }}>Entrar</button><button className="dark-btn" data-testid="signup-open-button" onClick={() => openRegister("client")}>Criar conta <ArrowRight size={16}/></button></>}</div>
    </nav>
    ...
    {publicProviderId && <PublicProviderProfile providerId={publicProviderId} user={user} onBack={openHome} onLogin={() => setAuth("login")} onMessage={setMessage}/>}    
    {!publicProviderId && view === "welcome" && <Welcome onChoose={openRegister} onLogin={() => setAuth("login")} />}
    ...
    {!publicProviderId && view === "profile" && <Profile user={user} logout={logout} onBack={() => { setMessage(""); setView("home"); }} onMessage={setMessage} onUserUpdate={setUser} />}
...
function Profile({user, logout, onBack, onMessage, onUserUpdate}) {
  ...
  return <>{switcher}{mode === "provider" && canUse(user, "provider") ? <ProviderWorkspace user={user} onBack={onBack} onMessage={onMessage} logout={logout}/> : <ClientWorkspace user={user} onBack={onBack} onMessage={onMessage} logout={logout} canActivateProvider={!canUse(user, "provider")} onActivateProvider={activateProvider} activatingProvider={activating}/>}</>;
}
...
function PublicProviderProfile({providerId, user, onBack, onLogin, onMessage}) {
